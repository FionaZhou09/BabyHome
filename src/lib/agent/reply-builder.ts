...
import { detectDiaperTopic, formatBabyAgeLabel } from "./intent";
import { withCrisisResources } from "./resource-pusher";
import type {
  ActivitySummary,
  Language,
...
export function summarizeActivities(activities: DemoActivity[]): ActivitySummary {
...
function activityContextLine(activities: DemoActivity[], language: Language) {
...
function patternContextLine(patternAnalysis: BabyPatternAnalysis, language: Language) {
...
function sourceLine(cards: BabyCareKnowledgeCard[], language: Language) {
...
function emotionalStateLine(emotion: EmotionDetectionResult, language: Language) {
  if (language === "zh") {
...
function babyUrgentReply(language: Language) {
  if (language === "zh") {
    return [
...
function parentCrisisReply(language: Language) {
  if (language === "zh") {
    return [
...
function emotionalReply(activities: DemoActivity[], language: Language) {
...
function feedingReply(activities: DemoActivity[], language: Language) {
...
function diaperReply(activities: DemoActivity[], language: Language) {
...
function poopReply(message: string, language: Language) {
...
function wetDiaperReply(activities: DemoActivity[], language: Language) {
  const diaperCount = activities.filter((activity) => activity.category === "diaper").length;
...
function cryingReply(language: Language) {
  if (language === "zh") {
    return [
...
function patternReply(activities: DemoActivity[], language: Language) {
...
function generalReply(activities: DemoActivity[], language: Language) {
  if (language === "zh") {
    return [
      "我可以陪你一起把问题变小一点。",
...
export function generateReplyFromContext(context: ParentSupportContext) {
  const {
    cleanMessage,
    language,
    emotion,
  ...
  if (intent === "emotional") {
    return withCrisisResources(
      emotionalReply(logContext.activities, language),
      crisisResourceNeed,
      language
    );
  }
  ...
  return withCrisisResources(
    generalReply(logContext.activities, language),
    crisisResourceNeed,
    language
  );
}
